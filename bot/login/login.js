...
async function startBot(_0x3cad9e) {
  ...
  // ─── FCA integrity check ────────────────────────────────────────────────
  // There must be exactly one FCA implementation in this project: the
  // vendored folder at /fca (built from fca-hridoy-main). This log line
  // exists specifically so a future accidental second FCA (e.g. someone
  // re-adding "hridoy-fca" to package.json, or restoring the old vendored
  // copy) is caught immediately at startup instead of causing a silent,
  // hours-later reaction-detection failure.
  const fcaResolvedPath = require.resolve("../../fca");
  const fcaVersion = (() => { try { return JSON.parse(require("fs").readFileSync(require("path").join(process.cwd(), "fca", "package.json"), "utf8")).version; } catch { return "unknown"; } })();
  log.warn("FCA", `Loaded FCA v${fcaVersion} from: ${fcaResolvedPath}`);
  if (fcaResolvedPath.includes("node_modules")) {
    log.err("FCA", "FCA was resolved from node_modules instead of the vendored ./fca folder — " +
      "this means the project is NOT using the audited/patched implementation. " +
      "Check for a stray \"hridoy-fca\" entry in package.json or node_modules.");
  }
  ...
  try {
    ...
    try {
      ...
      atomicJson.writeJsonAtomicSync(_0x41cca2, _0x372cb5, { spaces: 2 });
  ...
  (function _0x3592ba(_0x812929) {
    ...
    // ReactionRegistry behaves exactly like a Map for every existing command
    // (.set/.get/.delete/.size all work the same) but prevents this Map from
    // growing forever if a command's onReaction handler is set up but never
    // cleaned up. Default TTL is 24h so slow-to-react users are unaffected;
    // override via global.GoatBot.config.onReactionTTLHours if needed.
    if (global.GoatBot.onReaction && typeof global.GoatBot.onReaction.stopAutoSweep === "function") {
      global.GoatBot.onReaction.stopAutoSweep();
    }
    global.GoatBot.onReaction = new ReactionRegistry({
      ttlMs: ((global.GoatBot.config && global.GoatBot.config.onReactionTTLHours) || 24) * 60 * 60 * 1000
    });
    global.GoatBot.onReaction.startAutoSweep(15 * 60 * 1000, log);
